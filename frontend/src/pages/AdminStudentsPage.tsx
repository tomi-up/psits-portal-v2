      ...
      <div className="lg:pl-64">
        <header className="flex items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-6 py-4 lg:px-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-slate-900 dark:text-white">Student Management</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">View and manage the PSITS student roster</p>
            </div>
          </div>
          <AdminProfileMenu />
        </header>
                  ...
                  <tbody>
                    {paged.map((s) => (
                      <tr key={s.id} className="border-b border-slate-50 dark:border-slate-800/60 last:border-0">
                        <td className="px-5 py-3 font-mono text-xs text-slate-600 dark:text-slate-300">{s.student_id}</td>
                        <td className="px-5 py-3 text-slate-900 dark:text-white">
                          {s.last_name}, {s.first_name}
                          {s.middle_name ? ` ${s.middle_name}` : ''}
                        </td>
                        <td className="px-5 py-3 text-slate-500 dark:text-slate-400">{s.program ?? '—'}</td>
...
function StandingBadge({ standing }: { standing: string | null }) {
  if (standing === 'OVER_STAY') {
    return (
      <span className="rounded-full bg-amber-50 dark:bg-amber-950/40 px-2.5 py-1 text-xs font-medium text-amber-700 dark:text-amber-400">Over Stay</span>
    )
  }
  if (standing === 'IRREGULAR') {
    return <span className="rounded-full bg-rose-50 dark:bg-rose-950/40 px-2.5 py-1 text-xs font-medium text-rose-700 dark:text-rose-400">Irregular</span>
  }
  return <span className="rounded-full bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:text-emerald-400">Regular</span>
}
